...
const TOKEN = document.querySelector('meta[name="asm-token"]')?.content || document.querySelector('meta[name="csm-token"]')?.content || '';
...
function render() {
  ...
  document.title = attn ? `(${attn}) AGY Sessions` : 'AGY Sessions';
...
$('#btnSaveTpl').onclick = async () => {
  ...
  const list = await loadTemplates();
  list.push({ name, cwd: f.cwd.value.trim(), model: f.model.value, mode: f.mode.value, effort: f.effort?.value || '', extra: f.extra.value.trim(), worktree: f.worktree.checked, prompt: f.prompt.value, group: f.group.value.trim() });
  try { await api('PUT', '/api/templates', list); toast(`${t('Modèle enregistré')} : ${name}`); } catch (e) { toast(e.message, true); }
...
$('#btnBrowse').onclick = async () => {
  ...
  try {
    ...
    const path = window.asmNative?.pickFolder ? await window.asmNative.pickFolder(initial) : (window.csmNative ? await window.csmNative.pickFolder(initial) : (await api('POST', '/api/pick-folder', { initial })).path);
    if (path) {
      f.cwd.value = path;
