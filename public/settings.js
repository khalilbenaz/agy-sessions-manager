...
(() => {
  ...
  async function loadDiag() {
    ...
    try {
      ...
      const app = (window.asmNative || window.csmNative)?.appVersion?.();
      diagText = [
        `AGY Sessions ${app || d.app} (${t('serveur')} ${d.app}, ${d.runtime})`,
        `${t('Système')} : ${d.platform}`,
        `agy : ${d.agy?.ok ? '✓ ' + d.agy.version : '✗ ' + t('introuvable ou en erreur')} — ${d.agy?.path}`,
      ...
      out.classList.toggle('bad', !d.agy?.ok);
  ...
  const UPD_LABEL = {
    ...
    uptodate: 'AGY Sessions est à jour.', disabled: 'Mises à jour automatiques désactivées dans les réglages.',
