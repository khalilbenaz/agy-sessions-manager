...
// Prompts de démarrage pour Antigravity
...
function cleanModelName(m) {
  if (!m) return '';
  const match = String(m).match(/^(gemini-\d+\.\d+-(?:flash|pro))-(high|medium|low)$/);
...
module.exports = function (ctx) {
  ...
  const get = () => {
    const raw = { ...DEFAULTS, ...load(F.settings, {}) };
    if (raw.defaultModel) raw.defaultModel = cleanModelName(raw.defaultModel);
    if (!ENUMS.defaultEffort.includes(raw.defaultEffort)) raw.defaultEffort = 'medium';
    return raw;
  };
  ctx.getSettings = get;
